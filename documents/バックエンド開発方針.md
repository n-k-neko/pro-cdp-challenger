...
- 外部WebAPIを利用する際、レスポンスとドメインエンティティが一致しない場合は、`adapter`層に**レスポンスを受け取るためのクラス**と**マッピング用のインタフェースおよびクラス**を設け、ドメインエンティティの純粋性を確保する。
  - 利用する外部WebAPIの変更、レスポンスの仕様変更などへの対処を`adapter`層で完結させ、ドメインエンティティへの影響を防ぐ。
  - 例えば、`Foo`と`Bar`というフィールドのみをドメインエンティティで持ちたいと考えているが、`Hoge`なども併せて応答する外部WebAPIを利用する場合は、`当該3項目を有するレスポンス用クラス`と`当該2項目のみを有するドメインエンティティクラス`の2つを用意する。
  ...
  **レスポンス変換ロジック（具体的なMapper）**：上記インタフェースを実装
  ...
  public class IssResponseMapper implements ResponseMapper<IssResponse, IssDomainObject> {
      ...
      public Class<IssResponse> getResponseType() {
          return IssResponse.class; // このマッパーが対応するレスポンス型
      }
  }
  ```
  **WebClient**：上記変換ロジックを利用して、`adapter`層でレスポンスをドメインオブジェクトにマッピング。
  ...
  @Override
  public <R> Mono<R> fetchData(String endpoint, Class<R> domainType) {
      ResponseMapper<?, R> mapper = findMapper(domainType);

      return webClient.get()
              .uri(endpoint)
              .retrieve()
              .bodyToMono(mapper.getResponseType()) // 対応するレスポンス型を取得してリクエスト
              .map(response -> mapper.mapResponse(response)); // ドメインオブジェクトに変換
  }
